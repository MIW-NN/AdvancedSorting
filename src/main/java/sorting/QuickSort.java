 ...
 * Sort any List using quick sort
 */
    ...
    public static <L extends Comparable<? super L>> void sort(List<L> list, boolean debug) {
        ...
        while (leftIndex <= rightIndex) {
            ...
            if (leftIndex < rightIndex) {
                // if they haven't crossed over, swap the items so that the small element ends op on the left and the big one on the right
                Collections.swap(list, leftIndex, rightIndex);
                leftIndex++;
            } else {
                // if they have crossed over leftIndex should be pointing to the first element of the elements bigger than the pivot
